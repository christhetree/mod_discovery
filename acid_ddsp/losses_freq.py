...
import torch
import torchaudio
from auraloss.perceptual import SumAndDifference, FIRFilter
...
class SpectralConvergenceLoss(torch.nn.Module):
    ...
    def __init__(self, delta_win_len: Optional[int] = None):
        super().__init__()
...
class STFTMagnitudeLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        distance="L1",
        reduction="mean",
        delta_win_len: Optional[int] = None,
    ):
...
class STFTLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        if delta_win_ms is not None:
            assert sample_rate is not None, "delta_win_ms requires sample_rate"
            ...
            log.info(
                ...
                f"frame_rate: {frame_rate:.2f}, hop_len: {hop_size}"
            )

        self.spectralconv = SpectralConvergenceLoss(delta_win_len=self.delta_win_len)
        self.logstft = STFTMagnitudeLoss(
            log=True,
            reduction=reduction,
            distance=mag_distance,
            delta_win_len=self.delta_win_len,
        )
...
class DeltaMultiResolutionSTFTLoss(torch.nn.Module):
...
class SumAndDifferenceSTFTLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.w_diff = w_diff
        self.output = output
        self.mrstft = DeltaMultiResolutionSTFTLoss(
            fft_sizes,
            hop_sizes,
            win_lengths,
            window,
