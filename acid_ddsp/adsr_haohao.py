...
class DiffRoundFunc(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        return torch.round(input * 10**2) / (
            10**2
        )  # because 2 decimal point, 0.01 is the minimum ratio
...
class ADSREnvelopeShaper(nn.Module):
    def __init__(self, is_round_secs=False):
        ...
        self.round_decimal_places = (
            2  # because block size = 100, so min resolution = 1/ 100
        )
    ...
    def gen_envelope(
        ...
        """
        if floor is None:
            floor = torch.tensor([0.0]).unsqueeze(0).unsqueeze(-1)
            if device == "cuda":
                floor = floor.cuda()
        if peak is None:
            peak = torch.tensor([1.0]).unsqueeze(0).unsqueeze(-1)
            if device == "cuda":
                peak = peak.cuda()
        ...
        x[:, 0, :] = (
            1e-6  # offset 0 to epsilon value, so when attack = 0, first adsr value is not 0 but 1
        )
        ...
        return torch.clamp(signal, min=0.0, max=1.0)
    ...
    def forward(
        ...
        release_ratio = torch.tensor([0.0]).repeat(attack_secs.size(0), 1, 1)
        if device == "cuda":
        ...
        signal = self.gen_envelope(
            ...
            floor=None,
            peak=None,
            n_frames=int(total_secs * block_size),
            pow=2,
        )
...
def get_amp_shaper(
    shaper, onsets, attack_secs, decay_secs, sustain_level, offsets=None
):
    """
...
if __name__ == "__main__":
    ...
    for elem in [0.0, 0.001, 0.005, 0.01, 0.02]:
        attack_secs, decay_secs, sustain_level = (
            torch.tensor([0.2]),
            torch.tensor([elem]),
            torch.tensor([0.8]),
        )
