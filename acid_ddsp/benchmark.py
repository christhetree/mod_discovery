...
ac_config_path = os.path.join(CONFIGS_DIR, "audio_config_abstract_303.yml")
...
def main() -> None:
    ...
    f0_hz = tr.full((batch_size,), 440.0)
    ...
    filter_args = {"w_mod_sig": w_mod_sig, "q_mod_sig": q_mod_sig, "logits": logits}
    ...
    globals["synth"] = make_synth("AcidSynthLearnedBiquadCoeff", ac, batch_size)
    results.append(
        benchmark.Timer(
            stmt="y = synth(f0_hz, osc_shape, osc_gain, note_on_duration, filter_args, dist_gain, learned_alpha, phase); loss = y[1].sum(); loss.backward()",
            globals=globals,
    ...
    globals["synth"] = make_synth("AcidSynthLPBiquad", ac, batch_size)
    results.append(
        benchmark.Timer(
            stmt="y = synth(f0_hz, osc_shape, osc_gain, note_on_duration, filter_args, dist_gain, learned_alpha, phase); loss = y[1].sum(); loss.backward()",
            globals=globals,
    ...
    globals["synth"] = make_synth("AcidSynthLSTM", ac, batch_size, n_hidden=n_hidden)
    results.append(
        benchmark.Timer(
            stmt="y = synth(f0_hz, osc_shape, osc_gain, note_on_duration, filter_args, dist_gain, learned_alpha, phase); loss = y[1].sum(); loss.backward()",
            globals=globals,
